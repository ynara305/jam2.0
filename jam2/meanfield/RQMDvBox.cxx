...
RQMDvBox::RQMDvBox(Pythia8::Settings* s, InitialCondition* initcnd) : MCell(s),MeanField(s)
{
  initCell(initcnd);
  initBox(s);
}
...
void RQMDvBox::init(std::list<EventParticle*>& plist)
{
  firstEng=true;
}

void RQMDvBox::initBox(Pythia8::Settings* s)
{
  ...
  for(int x=0;x<maxX;x++) {
    ...
    mBox.push_back(new MBoxV(xmin,xmax,pos,edge,s));
...
void RQMDvBox::addParticle(std::list<EventParticle*>& plist,double t,int step)
{
  eFree=0.0;
  for(auto& i : plist) {
...
void RQMDvBox::evolution(list<EventParticle*>& plist,double t, double dt, int step)
{
  globalTime = t;
...
Vec4 RQMDvBox::computeEnergy(list<EventParticle*>& plist, int step)
{
  pTot=0.0;
